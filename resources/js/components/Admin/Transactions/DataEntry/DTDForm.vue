    ...
    <form @submit.prevent class="animate__animated animate__fadeIn">
        <div class="row justify-content-between">
            ...
            <div class="col-md-3 mb-2 text-center mr-13">
                <button type="button" class="btn-red">SEARCH REMITTER</button>
            </div>
        </div>
        <div class="row">
                        ...
                        <span v-else>
                            <span v-if="item.label == 'COUNTRY'">
                                <div class="form-floating vs">
                                    <v-select
                                        v-model="item.value"
                                        @option:selected="getStatesByCountry"
                                        :options="countries"
                            ...
                            <span v-else-if="item.label == 'STATE'">
                                <div class="form-floating vs" v-if="isState">
                                    <v-select
                                        v-model="item.value"
        ...
        <div class="row justify-content-between mt-5">
            ...
            <div class="col-md-3 mb-2 text-center mr-13">
                <button class="btn-red">SEARCH BENEFICIARY</button>
            </div>
        </div>
        <div class="row">
            <div class="px-4 col-md-12">
                        ...
                        <span v-else>
                            ...
                            <span v-else-if="item.label == 'CITY'">
                                <div class="form-floating vs">
                                    ...
                                    />
                                    <label for="city"
                                        >City/Municipality <span>*</span></label
                                    >
                                </div>
                            </span>
                            <span
            ...
            <div class="col-md-12 px-4 mt-3 mb-2">
                <div class="form-check">
                    <input
                        class="form-check-input"
                        @change="sameDelivery($event)"
                        type="checkbox"
                        value=""
                        id="flexCheckChecked"
                    />
                    ...
                    >
                        Delivery Address same as Present/Permanent Address?
                    </label>
                </div>
            </div>
            <div class="px-4 col-md-12">
                <div class="row g-1">
                    <div class="col-lg-5 mb-3">
                        <FloatingInput
                            v-model="form.confirm_address"
                            label="Delivery Address"
                            name="confirm_address"
                            :isInvalid="form.errors.has('confirm_address')"
                            :required="true"
                            textholder="No., Street, Village/Subdivision, Barangay"
                        ></FloatingInput>
                        <HasError :form="form" field="confirm_address" />
                    </div>
                    <div class="col-md-2 mb-3">
                        <div class="form-floating vs">
                            <v-select
                                v-model="form.confirm_country"
                                @option:selected="getStatesByCountry"
                                :options="countries"
                                placeholder="Select Country"
                                id="confirm_country"
                            />
                            <label for="confirm_country"
                                >Country <span>*</span></label
                            >
                        </div>
                        <HasError :form="form" field="confirm_country" />
                    </div>
                    <div class="col-md-2 mb-3" v-if="isState">
                        <div class="form-floating vs">
                            <v-select
                                v-model="form.confirm_state"
                                @option:selected="getCityByState"
                                :options="states"
                                placeholder="Select State"
                                id="confirm_states"
                            />
                            <label for="confirm_states"
                                >State <span>*</span></label
                            >
                        </div>
                        <HasError :form="form" field="confirm_states" />
                    </div>
                    <div class="col-md-2 mb-3" v-if="!isState">
                        <div class="form-floating vs">
                            <v-select
                                v-model="form.confirm_region"
                                @option:selected="getCityByState"
                                :class="{
                                    'is-invalid': form.errors.has('region'),
                                }"
                                :options="regions"
                                placeholder="Region"
                                id="confirm_region"
                            />
                            <label for="confirm_region"
                                >Region <span>*</span></label
                            >
                        </div>
                        <HasError :form="form" field="confirm_region" />
                    </div>
                    <div class="col-md-2 mb-3">
                        <div class="form-floating vs">
                            <v-select
                                v-model="form.confirm_city"
                                :options="cities"
                                placeholder="Select City/Municipality"
                                id="confirm_city"
                            />
                        ...
                        <HasError :form="form" field="confirm_city" />
                    </div>
                    <div class="col-lg-1 mb-3">
                        <FloatingInput
                            v-model="form.confirm_zip"
                            type="number"
                            label="Zip Code"
                            name="userEmail"
                            :isInvalid="form.errors.has('confirm_zip')"
                            :required="true"
                            textholder=""
                        ></FloatingInput>
                        <HasError :form="form" field="confirm_zip" />
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row g-2 remittance">
                ...
                <div class="col-md-2 mb-3">
                    <FloatingInput
                        ...
                        @keypress="isNumber()"
                        @keyup.enter="calculateRemittance"
                        type="text"
                        label="Amount"
    ...
    >
        ...
        <template v-slot:body>
            <div class="row py-3 justify-content-center">
                ...
                >
                    ...
                    >
                        ...
                        <label :for="item.label"
                            >{{ item.label }}
                            <span v-show="item.require == true">*</span></label
                        >
                    </div>
                    <DropFile
                        :class="{
                        ...
                        :fileType="item.type"
                        :name="item.label.split(' ').join('_').toLowerCase()"
            ...
            <div class="modal-button-holder justify-content-around">
                ...
                <div class="col">
                    <button type="button" @click="closeModal" class="btn-save">
                        OK
                    </button>
                </div>
            </div>
        ...
        <template v-slot:body>
            <div class="px-5 position-relative">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <GroupHalfInput
                            v-model="preview_confirm_address"
                            label="DELIVERY ADDRESS"
                            type="text"
                            :isDisabled="true"
                            name="company-name"
...
export default {
    ...
    data() {
        return {
            ...
            form: new Form({
                ...
                email: "dextermechanics@gmail.com",
                address: "BAGNAN",
                country: "",
                state: "",
                region: "",
    ...
    methods: {
        ...
        sameDelivery(event) {
            this.$nextTick(() => {
                if (event.target.checked) {
                    //get address from the fields
                    // console.log(
                    //     this.getCustomerCountry(this.beneficiary_fields)
                    // );
                    this.form.confirm_address =
                        this.getRowByFieldslabel(
                            this.beneficiary_fields,
                            "PRESENT/PERMANENT ADDRESS"
                        ).value ?? "";

                    this.form.confirm_country =
                        this.getRowByFieldslabel(
                            this.beneficiary_fields,
                            "COUNTRY"
                        ).value.label ?? "";

                    this.form.confirm_state =
                        this.getRowByFieldslabel(
                            this.beneficiary_fields,
                            "STATE"
                        ).value.label ?? "";

                    this.form.confirm_region =
                        this.getRowByFieldslabel(
                            this.beneficiary_fields,
                            "STATE"
                        ).value.label ?? "";

                    this.form.confirm_zip =
                        this.getRowByFieldslabel(
                            this.beneficiary_fields,
                            "ZIP CODE"
                        ).value ?? "";

                    this.form.confirm_city =
                        this.getRowByFieldslabel(
                            this.beneficiary_fields,
                            "CITY"
                        ).value.label ?? "";
        ...
        showSummary() {
            ...
            this.$Progress.start();
            this.form
                .post("/api/validate-dtd-transaction-fields")
                .then((data) => {
                    this.preview_transaction_name = "DOOR TO DOOR";
                    ...
                    this.preview_confirm_address = this.form.confirm_address;
        ...
        getRate(data) {
            ...
            console.log("rate", this.service_fee);

            this.fees.map((currency) => {
                if (currency.label.includes(data.label) == true) {
                    if (currency.amount_range == null) {
                        this.form.type_of_fee = currency.type_of_fee;
                    }
        ...
        calculateRemittance() {
            ...
            //get the selected provider here

            if (this.form.type_of_fee == "PERCENTAGE") {
                this.form.service_fee = amount * (this.form.service_fee / 100);
            }
        ...
        recalculateRemittance(event) {
            var amount = this.form.original_amount;
            if (event.target.value == "add") {
                this.form.net_amount = this.$filters.formatAmount(
                    amount * this.form.exchange_rate + this.form.service_fee,
                    2,
                    true
                );
            } else {
    ...
    async created() {
        // console.log("transaction type", this.$props.transactionTypeId);
        await axios
