                        ...
                        <div class="row g-2">
                            ...
                            <div class="col-md-3 mb-3">
                                <FloatingInput
                                    ...
                                    name="bank_account"
                                    :isInvalid="form.errors.has('bank_account')"
                                    placeholder=""
                                ></FloatingInput>
...
export default {
    ...
    methods: {
        ...
        showSummary() {
            ...
            this.$Progress.start();
            // this.form
            //     .post("/api/validate-cba-transaction-fields")
            //     .then((data) => {})
            //     .catch((e) => {
            //         console.log(e.message);
            //         // this.$Progress.fail();
            //     });
            this.preview_transaction_name = "CREDIT TO BANK ACCOUNT";

            //remitter
            this.preview_fullname = this.getCustomerName(this.remitter_fields);
            this.preview_remitter_details = this.form.remitter_details.fields;

            this.preview_mobile_number = this.getCustomerMobileNumber(
                this.remitter_fields
            );
            this.preview_gender = this.getCustomerGender(this.remitter_fields);
            this.preview_email = this.getCustomerEmailAddress(
                this.remitter_fields
            );
            this.preview_address = this.getCustomerAddress(
                this.remitter_fields
            );

            //bene

            // this.preview_bene_fullname = this.getCustomerName(
            //     this.beneficiary_fields
            // );

            // this.preview_beneficiary_details =
            //     this.form.beneficiary_details.fields;

            // this.preview_bene_gender = this.getCustomerGender(
            //     this.beneficiary_fields
            // );
            // this.preview_bene_mobile_number = this.getCustomerMobileNumber(
            //     this.beneficiary_fields
            // );

            // this.preview_bene_email = this.getCustomerEmailAddress(
            //     this.beneficiary_fields
            // );

            // this.preview_bene_address = this.getCustomerAddress(
            //     this.beneficiary_fields
            // );

            this.preview_bank_name = this.form.bank_name["label"];
            this.preview_bank_account = this.form.bank_account;
        ...
        getCustomerGender(customer_details) {
            return customer_details.find(
                (element) => element.label === "GENDER"
            ).value;
        },
        getCustomerMobileNumber(customer_details) {
            return customer_details.find(
                (element) => element.label === "MOBILE NUMBER"
            ).value;
        },
        getCustomerEmailAddress(customer_details) {
            return customer_details.find(
                (element) => element.label === "EMAIL ADDRESS"
            ).value;
        },
        getCustomerAddress(customer_details) {
            return customer_details.find(
                (element) => element.label === "PRESENT/PERMANENT ADDRESS"
            ).value;
        },
    },
    async created() {
